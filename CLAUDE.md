...
- **Default `SXT_RPC` is `wss://rpc.mainnet.sxt.network`.** Testnet is opt-in via env override. The publish *flow* mirrors the canonical [`spaceandtimefdn/sxt-chain-examples`](https://github.com/spaceandtimefdn/sxt-chain-examples/tree/main/tutorials/create_hello_world_table) tutorial (batchAll, NOT NULL, hyperKzg, EthEcdsaSigner) — but note the tutorial itself targets `wss://rpc.testnet.sxt.network`; our mainnet default is a deliberate divergence, not a mirror.
...
- **The seven real skills are:** `dataset-publish`, `index-contract`, `proof-of-sql-foundations`, `run-proven-query`, `chain-data-query`, `deploy-contract`, `pre-deploy-audit`. (`index-contract`'s CLI — `examples/scripts/index-contract.mjs` — registers SCI tables via `tables.createTableWithSciMetadata`; the resulting tables are not zk-provable until SXT promotes SCI to the proven surface.) Phantom skills referenced in old drafts (`verified-analytics`, `cross-chain-join`, `deploy-erc`, `post-deploy-monitor`, `biscuit-mint`, `queryrouter-call`) **do not exist** — don't reference them in new content.
